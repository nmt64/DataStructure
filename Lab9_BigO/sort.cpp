...
void quickSort(vector<int>::iterator front, vector<int>::iterator back) {
    ...
    int value = *front;		// pivot
    do {
		while (left != back && *left < value) ++left;	// JLS
...
// makes a copy of masterList into keyList, shuffles keyList, and then sorts keyList so that
// we can repeat the process a number of times with a different unsorted list each
// time.
...
int main(int argc, char **argv) {
    ...
    // measure the amount of time it takes to copy and shuffle the master list so
    ...
    for (int i = 0; i < numSorts; ++i) {
		keyList = masterList;
		random_shuffle(keyList.begin(), keyList.end());		// JLS
    }
    overhead.stop();
